...
class Ujian extends CI_Controller {
	...
	public function judul_skripsi()
	{
		...
		$transkip_nilai 		= $this->input->post('transkip_nilai', true);
		$krs_terakhir			= $this->input->post('krs_terakhir', true);
		$proposal_skripsi		= $this->input->post('proposal_skripsi', true);
		$jurnal_internasional3 	= $this->input->post('jurnal_internasional3', 	true);
		$jurnal_nasional3		= $this->input->post('jurnal_nasional3', true);


		if( $this->form_validation->run() === FALSE ){
			$data['status'] = false;
			$data['errors'] = [
				'transkip_nilai' 	=> form_error('transkip_nilai'),
				'krs_terakhir' 	=> form_error('krs_terakhir'),
				'proposal_skripsi' 	=> form_error('proposal_skripsi'),
				'jurnal_internasional3' 	=> form_error('jurnal_internasional3'),
				'jurnal_nasional3'		=> form_error('jurnal_nasional3'),
			];
		}else{
			$input = [
				'judul' 	=> $judul,
				'transkip_nilai' 	=> $transkip_nilai,
				'krs_terakhir' 	=> $krs_terakhir,
				'proposal_skripsi' 	=> $proposal_skripsi,
				'jurnal_internasional3' 		=> $jurnal_internasional3,
				'jurnal_nasional3' 		=> $jurnal_nasional3,
			];
			if($method === 'add'){
				$input['judul']	= $judul;
				$action = $this->master->create('judul_skripsi', $input);
			}else if($method === 'edit'){
				$judul = $this->input->post('judul', true);
				$action = $this->master->update('judul_skripsi', $input, 'judul', $judul);
			}
			$data['status'] = $action ? TRUE : FALSE;
		}
		$this->output_json($data);
    ...
    // {
    //     // $this->datatables->select('judul_skripsi');
    ...
    //     // if($id !== null){
    //     //     $this->datatables->where('judul_skripsi=', $id);
    //     // }
		

	// 	$this->load->model("Ujian_model");
	// 	$this->Ujian_model->tambahdata();
