<!-- Left side column. contains the logo and sidebar -->
<aside class="main-sidebar">

	<!-- sidebar: style can be found in sidebar.less -->
	<section class="sidebar">

		<!-- Sidebar user panel (optional) -->
		<div class="user-panel">
			<div class="pull-left image">
				<img src="<?=base_url()?>assets/dist/img/user1.png" class="img-circle" alt="User Image">
			</div>
			<div class="pull-left info">
				<p><?=$user->username?></p>
				<small><?=$user->email?></small>
			</div>
		</div>
		
		<ul class="sidebar-menu" data-widget="tree">
			<li class="header">MAIN MENU</li>
			<!-- Optionally, you can add icons to the links -->
			<?php 
			$page = $this->uri->segment(1);
			$master = ["jurusan", "kelas", "matkul", "dosen", "mahasiswa", "upload", "ujian"];
			$relasi = ["kelasdosen", "jurusanmatkul", "upload"];
			$users = ["users"];
			?>
			<li class="<?= $page === 'dashboard' ? "active" : "" ?>"><a href="<?=base_url('dashboard')?>"><i class="fa fa-dashboard"></i> <span>Dashboard</span></a></li>
			<?php if($this->ion_auth->is_admin()) : ?>
			<li class="treeview <?= in_array($page, $master)  ? "active menu-open" : ""  ?>">
				<a href="#"><i class="fa fa-folder"></i> <span>Data Master</span>
					<span class="pull-right-container">
						<i class="fa fa-angle-left pull-right"></i>
					</span>
				</a>
				<ul class="treeview-menu">
					<li class="<?=$page==='jurusan'?"active":""?>">
						<a href="<?=base_url('jurusan')?>">
							<i class="fa fa-circle-o"></i> 
							Master Jurusan
						</a>
					</li>
					<li class="<?=$page==='kelas'?"active":""?>">
						<a href="<?=base_url('kelas')?>">
							<i class="fa fa-circle-o"></i>
							Master Kelas
						</a>
					</li>
					<li class="<?=$page==='matkul'?"active":""?>">
						<a href="<?=base_url('matkul')?>">
							<i class="fa fa-circle-o"></i>
							Master Mata Kuliah
						</a>
					</li>
					<li class="<?=$page==='dosen'?"active":""?>">
						<a href="<?=base_url('dosen')?>">
							<i class="fa fa-circle-o"></i>
							Master Dosen
						</a>
					</li>
					<li class="<?=$page==='mahasiswa'?"active":""?>">
						<a href="<?=base_url('mahasiswa')?>">
							<i class="fa fa-circle-o"></i>
							Master Mahasiswa
						</a>
					</li>
					
				</ul>
			</li>
			<li class="treeview <?= in_array($page, $relasi)  ? "active menu-open" : ""  ?>">
				<a href="#"><i class="fa fa-link"></i> <span>Relasi</span>
					<span class="pull-right-container">
						<i class="fa fa-angle-left pull-right"></i>
					</span>
				</a>
				<ul class="treeview-menu">
					<li class="<?=$page==='kelasdosen'?"active":""?>">
						<a href="<?=base_url('kelasdosen')?>">
							<i class="fa fa-circle-o"></i>
							Kelas - Dosen
						</a>
					</li>
					<li class="<?=$page==='jurusanmatkul'?"active":""?>">
						<a href="<?=base_url('jurusanmatkul')?>">
							<i class="fa fa-circle-o"></i>
							Jurusan - Mata Kuliah
						</a>
					</li>
					
				</ul>
			</li>

			<!-- BAGIAN DOSEN -->

			<?php endif; ?>
			<?php if( $this->ion_auth->is_admin() || $this->ion_auth->in_group('dosen') ) : ?>
			<li class="<?=$page==='soal'?"active":""?>">
				<a href="<?=base_url('soal')?>" rel="noopener noreferrer">
					<i class="fa fa-file-text-o"></i> <span>Bank Soal</span>
				</a>
			</li>
			
			<?php endif; ?>
			<?php if( $this->ion_auth->in_group('dosen') ) : ?>
			<li class="<?=$page==='ujian'?"active":""?>">
				<a href="<?=base_url('ujian/master')?>" rel="noopener noreferrer">
					<i class="fa fa-chrome"></i> <span>Ujian</span>
				</a>
			</li>

			
			<?php endif; ?>
			<?php if( $this->ion_auth->in_group('dosen') ) : ?>
			<li class="treeview <?=$page==='upload'?"active":""?>">
				<a href="#"><i class="fa fa-link"></i> <span>Skripsi Mahasiswa</span>
					<span class="pull-right-container">
						<i class="fa fa-angle-left pull-right"></i>
					</span>
				</a>
				<ul class="treeview-menu">
					<li class="<?=$page==='upload'?"active":""?>">
						<a href="<?=base_url('upload/dsn_judulskripsi')?>">
							<i class="fa fa-circle-o"></i>
							Pengajuan Judul Skripsi
						</a>
					</li>
					<li class="<?=$page==='upload'?"active":""?>">
						<a href="<?=base_url('upload/dsn_seminar')?>">
							<i class="fa fa-circle-o"></i>
							Pengajuan Seminar Proposal 
						</a>
					</li>
					<li class="<?=$page==='upload'?"active":""?>">
						<a href="<?=base_url('upload/dsn_ujianskripsi')?>">
							<i class="fa fa-circle-o"></i>
							Pendaftaran Ujian Skripsi 
						</a>
					</li>
					</ul>
		</li>

		<?php endif; ?>
			<?php if( $this->ion_auth->in_group('dosen') ) : ?>
			<li class="<?=$page==='ujian'?"active":""?>">
				<a href="<?=base_url('ujian/datamhs')?>" rel="noopener noreferrer">
					<i class="fa fa-users"></i> <span>Data Mahasiswa</span>
				</a>
			</li>


				<!-- BAGIAN MAHASISWA -->

			<?php endif; ?>
			<?php if( $this->ion_auth->in_group('mahasiswa') ) : ?>
			<li class="<?=$page==='ujian'?"active":""?>">
				<a href="<?=base_url('ujian/list')?>" rel="noopener noreferrer">
					<i class="fa fa-chrome"></i> <span>Ujian</span>
				</a>
			</li>

			

			<?php endif; ?>
			<?php if( $this->ion_auth->in_group('mahasiswa') ) : ?>
			<li class="treeview <?=$page==='ujian'?"active":""?>">
				<a href="#"><i class="fa fa-folder"></i> <span>Data Mahasiswa</span>
					<span class="pull-right-container">
						<i class="fa fa-angle-left pull-right"></i>
					</span>
				</a>
				<ul class="treeview-menu">
					<li class="<?=$page==='upload'?"active":""?>">
						<a href="<?=base_url('upload/mhs_judulskripsi')?>">
							<i class="fa fa-circle-o"></i>
							Judul Skripsi
						</a>
					</li>
					<li class="<?=$page==='upload'?"active":""?>">
						<a href="<?=base_url('upload/mhs_seminar')?>">
							<i class="fa fa-circle-o"></i>
							 Seminar Proposal 
						</a>
					</li>
					<li class="<?=$page==='upload'?"active":""?>">
						<a href="<?=base_url('upload/mhs_ujianskripsi')?>">
							<i class="fa fa-circle-o"></i>
							 Ujian Skripsi 
						</a>
					</li>
				</ul>
			</li>

			<?php endif; ?>
			<?php if( $this->ion_auth->in_group('mahasiswa') ) : ?>
			<li class="<?=$page==='ujian'?"active":""?>">
				<a href="<?=base_url('upload/dospem')?>" rel="noopener noreferrer">
					<i class="fa fa-users"></i> <span>Dosen Pembimbing</span>
				</a>
			</li>


			<?php endif; ?>
			<?php if( $this->ion_auth->is_admin() ) : ?>
			<li class="<?=$page==='mahasiswa'?"active":""?>">
				<a href="<?=base_url('upload/mhs_judulskripsi')?>" rel="noopener noreferrer">
					<i class="fa fa-file-text-o"></i> <span>Pengajuan Judul Skripsi</span>
				</a>
			</li>
			<?php endif; ?>
			<?php if( $this->ion_auth->is_admin() ) : ?>
			<li class="<?=$page==='mahasiswa'?"active":""?>">
				<a href="<?=base_url('upload/mhs_seminar')?>" rel="noopener noreferrer">
					<i class="fa fa-file-text-o"></i> <span>Pengajuan Seminar Proposal</span>
				</a>
			</li>
			<?php endif; ?>
			<?php if( $this->ion_auth->is_admin() ) : ?>
			<li class="<?=$page==='mahasiswa'?"active":""?>">
				<a href="<?=base_url('upload/mhs_ujianskripsi')?>" rel="noopener noreferrer">
					<i class="fa fa-file-text-o"></i> <span>Pendaftaran Ujian Skripsi </span>
				</a>
			</li>

			<?php endif; ?>
			<?php if($this->ion_auth->is_admin()) : ?>
			<li class="header">ADMINISTRATOR</li>
			<li class="<?=$page==='hasilujian'?"active":""?>">
				<a href="<?=base_url('hasilujian')?>" rel="noopener noreferrer">
					<i class="fa fa-file"></i> <span>Hasil Ujian</span>
				</a>
			</li>
			<li class="<?=$page==='users'?"active":""?>">
				<a href="<?=base_url('users')?>" rel="noopener noreferrer">
					<i class="fa fa-users"></i> <span>User Management</span>
				</a>
			</li>
			<li class="<?=$page==='settings'?"active":""?>">
				<a href="<?=base_url('settings')?>" rel="noopener noreferrer">
					<i class="fa fa-cog"></i> <span>Settings</span>
				</a>
			</li>
			<?php endif; ?>
		</ul>

	</section>
	<!-- /.sidebar -->
</aside>